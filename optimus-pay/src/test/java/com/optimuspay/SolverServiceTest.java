...
public class SolverServiceTest {
    ...
    public void setUp() {
        ...
        orders = List.of(
            new Order("ORDER1", 30.0, List.of("KARTA1")),
            new Order("ORDER2", 40.0, List.of("KARTA2")),
            new Order("ORDER3", 10.0, List.of("KARTA1", "KARTA2")),
            new Order("ORDER4", 50.0, List.of())
        );
    }
    ...
    public void testEachOrderHasExactlyOnePayment() throws Exception {
        ...
        for (Order order : orders) {
            List<Variant> variants = result.stream()
                .filter(variant -> variant.getOrderId().equals(order.getId()))
                .toList();
    ...
    public void testDistributePoints() throws Exception {
        Variant variant = new Variant(new HashMap<>(Map.of(
            "PUNKTY", 30.0,
            "KARTA", 70.0
        )), 0.0, "order1");
